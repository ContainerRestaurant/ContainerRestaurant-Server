...
=== 푸시 토큰 제거
`DELETE /api/user/[유저_ID]/push/token`

operation::unregister-user-push-token[snippets='curl-request,http-response']

실제 푸시 토큰이 아닌 유저 테이블의 푸시 토큰 참조만 제거합니다.
